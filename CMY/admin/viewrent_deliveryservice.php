 ...
 if (isset($_GET['shifted'])) {
  ...
  <?php 
           ...
           $userid="";
                if (isset($_GET['delid'])) {
                	$delid=$_GET['delid'];
                	$delquery="delete from  rent_deliveryservice where deliser_id='$delid'";
                	$delquery1="delete from cus_orderent where orderId='$delid'";
				  ...
				  if (isset($_GET['confirm'])) {
                	$confirm=$_GET['confirm'];
					 ...
					 $query = "SELECT * FROM rent_deliveryservice WHERE deid_id ='$confirm'";
					 $msg=$db->select($query);
					  if ($msg) {
					...
					$query = "UPDATE rent_deliveryservice
					SET
					deliser_role  = '1'
					WHERE deid_id = $confirm";
					
					
					$query1 = "UPDATE cus_orderent
					SET
					status  = '1'
					WHERE orderId = $sager";
					
					
					$query2 = "UPDATE rent_payment
					SET
					rent_status  = '1'
					WHERE userId = $userid";
...
<a style="background:#6F9BC0;padding:5px;color:white;text-decoration:none;border-radius:10px;font-weight:bold;margin-left:40px;border:4px solid #0570B5;" href="rent_orderlist.php"><span class="glyphicon glyphicon-home" ></span> Back</a>
			...
			<div class="table-responsive">
                ...
                <div class="block">        
				   ...
				   <table id="datatable3" style="margin-left:40px;" class="table table-striped table-bordered table-hover" id="example">
					...
					<tbody>
					 <?php 
				    $query = "SELECT * FROM rent_deliveryservice";
					$msg=$db->select($query);
					if ($msg) {
						$i=0;
						while ($result=$msg->fetch_assoc()) {
					 ...
					 ?>
						<tr class="primary">
						  <td  style="color:black;text-align:center;font-weight:bold;font-size:12px;"><a href="rent_orderlist.php"><?php echo($result['deliser_name']) ;?></a></td>
